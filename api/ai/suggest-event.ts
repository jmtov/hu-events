...
import type { EventSuggestion, SuggestEventPayload } from '../../src/types/event-suggestion.js'
...

{
  ...
  "expected_attendees": <number or null>,
  "checklist": [
...
### expected_attendees
- Extract from the description if a number of people, team size, or participant count is explicitly mentioned (e.g. "50 pessoas", "team of 30", "100 attendees")
- Return null if no clear number is mentioned — do not guess
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    return res.status(502).json({ message: 'Invalid JSON from Gemini' })
  }
  ...
  const suggestion: EventSuggestion = {
    ...
    expected_attendees: typeof parsed.expected_attendees === 'number' ? Math.round(parsed.expected_attendees) : null,
    checklist: Array.isArray(parsed.checklist) ? parsed.checklist : [],
    budget_estimates:
      typeof parsed.budget_estimates === 'object' && parsed.budget_estimates !== null
        ? Object.fromEntries(
            Object.entries(parsed.budget_estimates as Record<string, unknown>).map(([k, v]) => [
              k,
