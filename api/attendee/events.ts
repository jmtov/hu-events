...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const eventIds = myParticipations.map(
      (p: { event_id: string }) => p.event_id,
    );

    const [{ data: events }, { data: checklistItems }, { data: completions }] =
      await Promise.all([
        supabase
          .from('events')
          .select('id, title, event_type, date_start, date_end, location')
          .in('id', eventIds),
        supabase
        ...
        supabase
          ...
          .in(
            'participant_id',
            myParticipations.map((p: { id: string }) => p.id),
          )
          .eq('completed', true),
      ]);
    ...
    for (const c of completions ?? []) {
      ...
      completionCountMap[completion.participant_id] =
        (completionCountMap[completion.participant_id] ?? 0) + 1;
    }
    const _completions = await supabase
      .from('participant_checklist_items')
      .select('participant_id, checklist_item_id')
      .in(
        'participant_id',
      ...
      .map(
        (participant: {
          ...
          const event = (events ?? []).find(
            (e: { id: string }) => e.id === participant.event_id,
          ) as
            | {
                id: string;
                title: string;
          ...
          return {
            ...
            date_end: event.date_end ?? null,
            location: event.location ?? null,
            rsvp_status: participant.rsvp_status as
              | 'pending'
              | 'confirmed'
              | 'declined',
