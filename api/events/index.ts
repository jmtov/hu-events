...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (req.method === 'GET') {
    ...
    return res
      .status(200)
      .json(
        (events ?? []).map((e) => ({ ...e, rsvp_count: rsvpMap[e.id] ?? 0 })),
      );
  }
  ...
  if (req.method === 'POST') {
      ...
      if (modules.budget && body.budget) {
        writeBudgets([
          ...readBudgets(),
          {
            event_id: eventId,
            currency: 'USD',
            categories: body.budget as never,
            updated_at: now,
          },
        ]);
      }
        ...
        body.triggers.map((t) => {
          if (t.source === 'checklist') {
            return {
              ...
              checklist_item_id:
                items?.find((ci) => ci.label === t.name)?.id ?? null,
              milestone_type: null,
    ...
    if (modules.budget && body.budget) {
      await supabase.from('budget_config').insert({
        event_id: eventId,
        currency: 'USD',
        categories: body.budget,
      });
    }
