...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    ...
    const [
      ...
      supabase.from('participants').select('*').eq('event_id', eventId).order('sort_order', { ascending: true }),
      supabase.from('checklist_items').select('*').eq('event_id', eventId).order('sort_order', { ascending: true }),
      supabase.from('preference_fields').select('*').eq('event_id', eventId).order('sort_order', { ascending: true }),
      supabase.from('contacts').select('*').eq('event_id', eventId),
      supabase.from('budget_config').select('*').eq('event_id', eventId).maybeSingle(),
    ...
    const triggers = (dbTriggers ?? []).map((t: Record<string, unknown>) => {
      const name =
        t.source === 'checklist_item'
          ? ((checklist ?? []) as Record<string, unknown>[]).find((ci) => ci.id === t.checklist_item_id)?.label ?? ''
  ...
  if (req.method === 'PUT') {
    ...
    const { data: updated, error } = await supabase
      ...
      .select()
      .single()
    if (error || !updated) return res.status(error ? 500 : 404).json({ message: error?.message ?? 'Event not found' })
    ...
    if (modules.participantList) {
      ...
      if (body.participants?.length) {
        await supabase.from('participants').insert(
          body.participants.map((p, i) => ({
            event_id: eventId, email: p.email.trim(), full_name: '', rsvp_status: 'pending', sort_order: i,
          })),
        )
      }
  ...
  if (req.method === 'PATCH') {
    ...
    const { data: created, error } = await supabase
      .from('budget_config')
      .insert({ event_id: eventId, currency: 'USD', categories, updated_at: now })
      .select()
      .single()
    if (error) return res.status(500).json({ message: error.message })
    return res.status(200).json(created)
