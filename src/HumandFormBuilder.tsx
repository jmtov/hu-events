...
const PREVIEW = {
  ...
  number: ({ label }: { label: string }) => (
    <div>
      <label style={lbStyle}>{label}</label>
      <div style={{ ...inputBase, width: 100, color: '#94a3b8' }}>0</div>
    </div>
  ),
        ...
        {['Sim', 'Não'].map((o) => (
          <div
            ...
            style={{
              ...
              fontSize: 13,
              color: '#64748b',
              background: o === 'Sim' ? '#f0fdf4' : '#fff',
              fontWeight: o === 'Sim' ? 600 : 400,
            }}
          >
            {o}
          </div>
        ...
        {(options || []).map((o, i) => (
          <div
            ...
            style={{
              padding: '5px 14px',
              borderRadius: 20,
              border: `1.5px solid ${i === 0 ? '#c7d2fe' : '#e2e8f0'}`,
              fontSize: 12,
              color: '#64748b',
...
export default function App() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setError(
        'Erro ao gerar o formulário. Verifique a descrição e tente novamente.',
      );
    } finally {
      setLoading(false);
    }
  ...
  const activeFields: FormField[] = result
    ? result.fields.filter((_: FormField, i: number) => !removed.includes(i))
    : [];
  const counts: Record<string, number> =
    result && result.fields.length > 0
      ? Object.fromEntries(
          Object.keys(INPUT_CONFIG).map((k) => [
            k,
            activeFields.filter((f: FormField) => f.inputType === k).length,
          ]),
        )
      : {};
      ...
      <div style={{ maxWidth: 640, margin: '0 auto' }}>
        <div style={{ marginBottom: 28, textAlign: 'center' }}>
          ...
          <p style={{ color: '#64748b', fontSize: 14, margin: 0 }}>
            Gerador de formulário de preferências
          </p>
        </div>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button
              ...
              style={{
                ...
                border: 'none',
                background:
                  loading || !description.trim() ? '#c7d2fe' : '#6366f1',
                color: '#fff',
                fontWeight: 600,
                fontSize: 14,
                cursor:
            ...
            >
              {loading ? 'Gerando formulário...' : '✨ Gerar Formulário'}
            </button>
            {(result || description) && (
              <button
                onClick={reset}
          ...
          <div style={{ animation: 'fadeIn .4s ease' }}>
            {result!.fields.length === 0 ? (
              ...
              >
                ...
                >
                  A descrição não parece descrever um evento corporativo válido.
                  <br />
                  Tente detalhar melhor o evento.
                </p>
                <button
                  onClick={reset}
                  style={{
              ...
              <>
                ...
                >
                  ...
                  >
                    ...
                    >
                      {activeFields.length} campos
                    </span>
                    {Object.entries(INPUT_CONFIG).map(
                      ([k, v]) =>
                        counts[k] > 0 && (
                          <span
                            key={k}
                  ...
                  <button
                    ...
                    style={{
                      padding: '6px 14px',
                      borderRadius: 20,
                      border: `1.5px solid ${preview ? '#6366f1' : '#e2e8f0'}`,
                      background: preview ? '#eef2ff' : '#fff',
                      color: preview ? '#4f46e5' : '#64748b',
                      fontSize: 13,
                      fontWeight: 600,
                      cursor: 'pointer',
                    }}
                  >
                    {preview ? '✏️ Editar' : '👁️ Pré-visualizar'}
                  </button>
                </div>
                ...
                {preview ? (
                  ...
                  >
                    ...
                    >
                      Pré-visualização do formulário para participantes
                    </p>
                    <div
                      style={{
                        display: 'flex',
                    ...
                    >
                      {activeFields.map((f: FormField, i: number) => {
                        const Comp =
                          PREVIEW[f.inputType as keyof typeof PREVIEW] ||
                          PREVIEW.text;
                        return (
                          <div
                            key={i}
                  ...
                  >
                    ...
                    >
                      Clique no ✕ para remover campos que não se aplicam
                    </p>
                    {result!.fields.map((field: FormField, i: number) => {
                      if (removed.includes(i)) return null;
                      const cfg =
                        INPUT_CONFIG[
                          field.inputType as keyof typeof INPUT_CONFIG
                        ] || INPUT_CONFIG.text;
                      return (
                        <div
                          key={i}
