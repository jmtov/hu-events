...
const DraftItemRow = ({ item, onEdit, onDelete }: DraftItemRowProps) => (
  <div className="flex items-center gap-3 rounded-xl border border-border bg-card px-4 py-3 text-sm ring-1 ring-foreground/10">
    <div className="flex min-w-0 flex-1 items-center gap-3">
      <span className="truncate font-medium text-card-foreground basis-full">
        {item.name}
      </span>
      <span
        className={cn(
    ...
    <div className="flex shrink-0 items-center gap-2">
      {item.required && (
        <span
          className="text-xs text-muted-foreground inline-flex shrink-0 items-center rounded-md px-2 py-0.5 font-medium ring-1 ring-inset"
        >
          Required
        </span>
      )}
