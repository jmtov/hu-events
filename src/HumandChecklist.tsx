...
export default function App() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setError(
        'Erro ao gerar o checklist. Verifique a descrição e tente novamente.',
      );
    } finally {
      setLoading(false);
    }
  ...
  const toggleRequired = (i: number) =>
    setToggled((p) => ({ ...p, [i]: !p[i] }));
  const getRequired = (i: number, item: ChecklistItem) =>
    i in toggled ? toggled[i] : item.suggestedRequired;

  const counts: Counts | null =
    result && result.items.length > 0
      ? {
          task: result.items.filter((it: ChecklistItem) => it.type === 'task')
            .length,
          document_upload: result.items.filter(
            (it: ChecklistItem) => it.type === 'document_upload',
          ).length,
          info_input: result.items.filter(
            (it: ChecklistItem) => it.type === 'info_input',
          ).length,
          required: result.items.filter((it: ChecklistItem, i: number) =>
            getRequired(i, it),
          ).length,
        }
      : null;
      ...
      <div style={{ maxWidth: 620, margin: '0 auto' }}>
        <div style={{ marginBottom: 28, textAlign: 'center' }}>
          ...
          <p style={{ color: '#64748b', fontSize: 14, margin: 0 }}>
            Gerador de checklist pré-evento
          </p>
        </div>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            >
              {loading ? 'Gerando checklist...' : '✨ Gerar Checklist'}
            </button>
            {(result || description) && (
              <button
                onClick={reset}
              ...
              <>
                ...
                >
                  ...
                  >
                    ...
                    >
                      {result!.items.length} itens gerados
                    </span>
                    <span style={{ fontSize: 13, color: '#94a3b8' }}>·</span>
                    <span style={{ fontSize: 13, color: '#64748b' }}>
                      {counts!.required} obrigatórios
                    </span>
                  </div>
                  <div style={{ display: 'flex', gap: 8 }}>
                    {Object.entries(TYPE_CONFIG).map(
                      ([k, v]) =>
                        counts![k as keyof Counts] > 0 && (
                          <span
                            key={k}
                            style={{
                              fontSize: 12,
                ...
                >
                  Clique no ícone de cadeado para alterar a obrigatoriedade
                  sugerida
                </p>
                      ...
                      >
                        ...
                        <button
                          ...
                          style={{
                            ...
                            padding: '5px 10px',
                            borderRadius: 20,
                            border: `1.5px solid ${req ? '#fde68a' : '#e2e8f0'}`,
                            background: req ? '#fffbeb' : '#f8fafc',
                            cursor: 'pointer',
                            fontSize: 12,
                            fontWeight: 600,
                            color: req ? '#b45309' : '#94a3b8',
                            flexShrink: 0,
                          }}
                        >
                          <span>{req ? '🔒' : '🔓'}</span>
                    ...
                    {JSON.stringify(
                      {
                        items: result!.items.map(
                          (it: ChecklistItem, i: number) => ({
                            ...it,
                            suggestedRequired: getRequired(i, it),
                          }),
                        ),
                      },
                      null,
